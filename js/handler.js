...
if (previous_state !== null) {
    collection = JSON.parse(previous_state);
}
        ...
        function(data) {
            ...
            for (let z in data) {
                ...
                add_to_collection.addEventListener("click", function() {
                    ...
                    let x = P.getDropdown(list);
                    console.log(x);
                    dropdown_modal_content.innerHTML = x;
...
export function mobileBtnCB() {
    let str = Q.getMobileView();
    for (let name in collection) {
        str += Q.getSlideOutItem(name);
    }
    $("#mobile-demo").html(str);
    for (let name in collection) {
        let ele = document.getElementById(name);
        ele.addEventListener("click", function() {
            let temp = "";
